...
namespace SchoolEnterpriseManageSys.Utilities.DEncryptHelper
{
    public class AESCrypto
    {
        ...
        {
            password = (password ?? string.Empty).PadLeft(16, '0');
            string iv;
            using (MD5 md5 = MD5.Create())
            {
                iv = string.Join(string.Empty, md5.ComputeHash(Encoding.UTF8.GetBytes(password)).Select(c => c.ToString("X2")).Take(8));
            }
            //取密码的MD5值前16位为IV
        ...
        public static string Decrypt(string text, string password)
        {
            ...
            //取密码的MD5值前16位为IV
            return Decrypt(text, password, iv);
        }
