...
namespace SchoolEnterpriseManageSys.Utilities.CommomHelper
{
    ...
    {
        private static ConcurrentDictionary<string, string> _mimeTypes = new ConcurrentDictionary<string, string>();

        #region  Constructors and Destructor

        static MimeHelper()
        {
            var str = @"# MIME type			Extension
        ...
        {
            string result = null;
            if (!_mimeTypes.TryGetValue(extension, out result))
            {
                result = "unknown";
            }
