...
export class ArchivesCampusBaseEditComponent implements OnInit {
  ...
  // }
  getTypeDetail() {
    this.projectService.getProjectTypeDetail(4).then(res => {
      if (!res) return;
  ...
  getTypeList() {
    this.projectService.getProjectType().then(res => {
      if (!res) return;
      this.projectTypeList = res['result'].types;
    });
  }
  ...
  submit() {
    ...
    if (this.id) {
      input.id = this.id;
      this.projectService.updateCampusBase(input).then(res => {
        this.submitting = false;
        if (!res) return;
        this.msgSrv.success(`提交成功`);
        this.reuseTabService.clear();
        this.router.navigateByUrl('/archives/CampusBase');
      });
    } else {
      input.fileIdList = this.fileIdList;
      this.projectService.insertCampusBase(input).then(res => {
        this.submitting = false;
        if (!res) return;
        this.msgSrv.success(`提交成功`);
        this.reuseTabService.clear();
