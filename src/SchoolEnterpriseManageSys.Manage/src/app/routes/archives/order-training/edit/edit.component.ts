...
export class ArchivesOrderTrainingEditComponent implements OnInit {
  constructor(
    ...
    this.form = this.formBuilder.group({
      ...
      classStudentCount: [null, [Validators.required]],
  ...
  getTypeDetail() {
    this.projectService.getProjectTypeDetail(16).then(res => {
      if (!res) return;
  ...
  submit() {
    ...
    if (this.id) {
      input.id = this.id;
      this.projectService.updateOrderTraining(input).then(res => {
      ...
      this.projectService.insertOrderTraining(input).then(res => {
        this.submitting = false;
        if (!res) return;
        this.msgSrv.success(`提交成功`);
        this.reuseTabService.clear();
