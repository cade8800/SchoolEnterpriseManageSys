...
export class ArchivesOffCampusBaseEditComponent implements OnInit {
  ...
  getTypeDetail() {
    this.projectService.getProjectTypeDetail(2).then(res => {
      if (!res) return;
  ...
  submit() {
    ...
    if (this.id) {
      input.id = this.id;
      this.projectService.updateOffCampusBase(input).then(res => {
      ...
      this.projectService.insertOffCampusBase(input).then(res => {
        this.submitting = false;
        if (!res) return;
        this.msgSrv.success(`提交成功`);
        this.reuseTabService.clear();
