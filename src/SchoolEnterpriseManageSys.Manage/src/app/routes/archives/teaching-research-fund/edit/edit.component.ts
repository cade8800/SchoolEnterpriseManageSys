...
export class ArchivesTeachingResearchFundEditComponent implements OnInit {
  ...
  getTypeDetail() {
    this.projectService.getProjectTypeDetail(64).then(res => {
      if (!res) return;
  ...
  submit() {
    ...
    if (this.id) {
      input.id = this.id;
      this.projectService.updateTeachingResearchFund(input).then(res => {
      ...
      this.projectService.insertTeachingResearchFund(input).then(res => {
        this.submitting = false;
        if (!res) return;
        this.msgSrv.success(`提交成功`);
        this.reuseTabService.clear();
