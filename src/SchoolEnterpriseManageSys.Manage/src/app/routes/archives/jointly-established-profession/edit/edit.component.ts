...
export class ArchivesJointlyEstablishedProfessionEditComponent
  implements OnInit {
  constructor(
  ...
  getTypeDetail() {
    this.projectService.getProjectTypeDetail(256).then(res => {
      if (!res) return;
  ...
  submit() {
    ...
    if (this.id) {
      input.id = this.id;
      this.projectService.updateJointlyEstablishedProfession(input).then(res => {
        this.submitting = false;
        if (!res) return;
        this.msgSrv.success(`提交成功`);
        this.reuseTabService.clear();
        this.router.navigateByUrl('/archives/JointlyEstablishedProfession');
      });
    } else {
      input.fileIdList = this.fileIdList;
      this.projectService.insertJointlyEstablishedProfession(input).then(res => {
        this.submitting = false;
        if (!res) return;
        this.msgSrv.success(`提交成功`);
        this.reuseTabService.clear();
