...
export class ArchivesCoAuthoredBookOrCourseEditComponent implements OnInit {
  constructor(
    ...
    this.form = this.formBuilder.group({
      departmentId: '',
      coAuthoredType: [null, [Validators.required]],
  ...
  getTypeDetail() {
    this.projectService.getProjectTypeDetail(32).then(res => {
      if (!res) return;
  ...
  submit() {
    ...
    if (this.id) {
      input.id = this.id;
      this.projectService.updateCoAuthoredBookOrCourse(input).then(res => {
      ...
      this.projectService.insertCoAuthoredBookOrCourse(input).then(res => {
        this.submitting = false;
        if (!res) return;
        this.msgSrv.success(`提交成功`);
        this.reuseTabService.clear();
