...
export class AssessmentExpertEditComponent implements OnInit {
  constructor(
    ...
    this.assessmentDepartmentId = this.activatedRoute.snapshot.paramMap.get(
      'dep',
    );
    ...
    this.form = this.formBuilder.group({
      id: '',
      expertRatingScore: new FormControl(
        { value: '', disabled: !this.id },
        Validators.required,
      ),
      expertRating: new FormControl({ value: '', disabled: !this.id }),
    });
  }
  ...
  submit() {
    ...
    if (input.expertRatingScore > this.index.standardScore) {
    ...
    this.assessmentService.expertRating(input).then(res => {
